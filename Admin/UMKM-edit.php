...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    ...
    <?php
        ...
        $stmt = $conn->prepare("SELECT * FROM UMKM WHERE umkm_id = :umkm_id");
        ...
        $owner = isset($k['owner']) ? $k['owner'] : '';
        $phone_number = isset($k['phone_number']) ? $k['phone_number'] : '';
        ...
        if (isset($_POST['submit'])) {
            ...
            if (move_uploaded_file($gambar, $target_file)) {
                ...
                if (!empty($gambar_lama)) {
                    unlink("../UMKM/" . $gambar_lama);
                }

                // Proses update data di database
                $query = $conn->prepare("UPDATE umkm SET name=:name, gambar=:gambar, owner=:owner, phone_number=:phone_number, deskripsi=:deskripsi WHERE umkm_id=:umkm_id");
                ...
                if ($update) {
                    $_SESSION['msg'] = "UMKM berhasil diperbarui!";
                    header("Location: #");
                    exit();
                } else {
                    $_SESSION['msg'] = "Gagal memperbarui UMKM.";
                    header("Location: #");
                    exit();
                }
            } else {
            ...
            <div class="container" data-wow-delay="0.1s">
                ...
                <div class="box">
                    ...
                    <form action="" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-floating mb-3">
                            <input type="text" name="owner" placeholder="Owner" class="input-control" value="<?= $owner; ?>" required>
                        </div>
                        <div class="form-floating mb-3">
                            <input type="text" name="phone_number" placeholder="Nomer HP" class="input-control" value="<?= $phone_number; ?>" required>
                        </div>
                        <div class="form-floating mb-3">
